...
import {CreateTreeDto} from "../../domain/tree/dto/create-tree.dto";
import {JoinToTreeDto} from "../../domain/tree/dto/join-to-tree.dto";
import {UpdateTreeDto} from "../../domain/tree/dto/update-tree.dto";
...
export class RepositoryQuery {
    ...
    private _param: {
        new(): Partial<CreateTreeDto>;
        new(...args: any[]): Partial<CreateTreeDto>;
        call: (this: Function, thisArg: any, ...argArray: any[]) => void;
        caller: undefined;
        bind: (this: Function, thisArg: any, ...argArray: any[]) => void;
        apply: (this: Function, thisArg: any, argArray?: any) => void;
        function: void;
        name: string;
        length: number;
    ...
    public beginWithUser(id: string): RepositoryQuery {
      ...
      attachExternalEntitiesByParent(
        childEntity: string,
        parentEntity: string,
        attachArchived: boolean
      ): RepositoryQuery {
        const queryText = `
          MATCH ($parentEntity:$parentEntity)
          CALL apoc.path.spanningTree($parentEntity, {labelFilter:'>$childEntity', maxLevel:10}) YIELD path
          UNWIND nodes(path) as $childEntityList
          WHERE $childEntityList:$childEntity AND ($childEntityList.archived IS NULL OR $childEntityList.archived = $attachArchived)
        `;
        const params = {
          parentEntity,
          childEntity,
        ...
        this.query.raw(queryText, params);
        return this;
  ...
  ${query}      
    ...
    public resolveExternalRelations = (
        ...
        if (ids?.length > 0) {
            ...
            this.attachExternalEntitiesByParent(childEntity, parentEntity, true);
        }
        return this;
    };
    ...
    updateTree(treeToUpdate: any) {

        let query = `
        MATCH (n)
        WHERE ID(n) = ${treeToUpdate.id}
        SET n = ${JSON.stringify(treeToUpdate)}
        RETURN n
        `;
        this.query.raw(query);
        return this;
    ...
    createTree(createTreeDto: CreateTreeDto) {
        let query = `
        CREATE (n:Tree ${JSON.stringify(createTreeDto)})
        RETURN n
        `;
        this.query.raw(query);
        return this;
    ...
    joinToTreeMarried(id: number, joinToTreeProperty: JoinToTreeDto) {

        let query = `
        MATCH (n)
        WHERE ID(n) = ${id}
        SET n = ${JSON.stringify(joinToTreeProperty)}
        RETURN n
        `;
        this.query.raw(query);
        return this;
    ...
    joinToTreeDescendant(id: number, joinToTreeProperty: JoinToTreeDto) {
    ...
    joinToTreeMarriedSubTree(id: number, joinToTreeProperty: JoinToTreeDto) {
    ...
    updateTreeEntity(id: number, updateTreeDto: UpdateTreeDto) {

            let query = `
            MATCH (n)
            WHERE ID(n) = ${id}
            SET n = ${JSON.stringify(updateTreeDto)}
            RETURN n
            `;
            this.query.raw(query);
            return this;
